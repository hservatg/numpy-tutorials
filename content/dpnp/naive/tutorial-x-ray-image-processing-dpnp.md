...
Finally, check if GPU is available through Data Parallel Extension NumPy (dpnp):
...
if ":gpu:" in str(test_variable.device):
    ...
    print ("Failed to allocate `test_variable` on GPU")
    sys.exit(0)
```
...
# xray_image_sobel = np.hypot(x_sobel, y_sobel)                                           # Original code to be executed on the CPU
xray_image_sobel = dpnp.asnumpy(dpnp.hypot(dpnp.asarray(x_sobel), dpnp.asarray(y_sobel))) # DPNP version of hypot

# xray_image_sobel *= 255.0 / np.max(xray_image_sobel)                             # Original code to be executed on the CPU
xray_image_sobel *= 255.0 / dpnp.asnumpy(dpnp.max(dpnp.asarray(xray_image_sobel))) # DPNP version of max
```
...
# xray_image_canny = np.hypot(x_prewitt, y_prewitt)                                           # Original code to be executed on the CPU
xray_image_canny = dpnp.asnumpy(dpnp.hypot(dpnp.asarray(x_prewitt), dpnp.asarray(y_prewitt))) # DPNP version of hypot

# xray_image_canny *= 255.0 / np.max(xray_image_canny)                             # Original code to be executed on the CPU
xray_image_canny *= 255.0 / dpnp.asnumpy(dpnp.max(dpnp.asarray(xray_image_canny))) # DPNP version of max
...
#print("The minimum pixel value is: ", np.min(xray_image))                  # Original code to be executed on the CPU
print("The minimum pixel value is: ", dpnp.min(dpnp.asarray(xray_image)))   # DPNP version of min
#print("The maximum pixel value is: ", np.max(xray_image))                  # Original code to be executed on the CPU
print("The maximum pixel value is: ", dpnp.max(dpnp.asarray(xray_image)))   # DPNP version of max
#print("The average pixel value is: ", np.mean(xray_image))                 # Original code to be executed on the CPU
print("The average pixel value is: ", dpnp.mean(dpnp.asarray(xray_image)))  # DPNP version of mean
#print("The median pixel value is: ", np.median(xray_image))                # Original code to be executed on the CPU
print("The median pixel value is: ", dpnp.median(dpnp.asarray(xray_image))) # DPNP version of median
```
...
pixel_intensity_distribution = ndimage.histogram(
    # xray_image, min=np.min(xray_image), max=np.max(xray_image), bins=256                                                             # Original code to be executed on the CPU
    xray_image, min=dpnp.asnumpy(dpnp.min(dpnp.asarray(xray_image))), max=dpnp.asnumpy(dpnp.max(dpnp.asarray(xray_image))), bins=256   # DPNP version of min/max
)
...
# xray_image_mask_noisy = np.where(xray_image > 150, xray_image, 0)                                           # Original code to be executed on the CPU
xray_image_mask_noisy = dpnp.asnumpy(dpnp.where(dpnp.asarray(xray_image) > 150, dpnp.asarray(xray_image), 0)) # DPNP usage of where
...
# xray_image_mask_less_noisy = np.where(xray_image > 150, 1, 0)                             # Original code to be executed on the CPU
xray_image_mask_less_noisy = dpnp.asnumpy(dpnp.where(dpnp.asarray(xray_image) > 150, 1, 0)) # DPNP usage of where
