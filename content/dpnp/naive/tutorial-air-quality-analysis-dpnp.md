...
- [SciPy](https://scipy.org/install/) installed in your environment, and,

- Data Parallel Extensions for NumPy

Now, check if GPU is available through Data Parallel Extension NumPy (dpnp):
...
```{code-cell} ipython3
# np.all(np.isfinite(pollutant_data))                 # Original code to be executed on the CPU
dpnp.all(dpnp.isfinite(dpnp.asarray(pollutant_data))) # DPNP usage of isfinite and all
```
...
```{code-cell} ipython3
def moving_mean(a, n):
    # ret = np.cumsum(a, dtype=float, axis=0)                             # Original code to be executed on the CPU
    ret = dpnp.asnumpy(dpnp.cumsum(dpnp.asarray(a), dtype=float, axis=0)) # DPNP usage of cumsum
    ret[n:] = ret[n:] - ret[:-n]
    return ret[n - 1:] / n
...
```{code-cell} ipython3
# pollutants = np.concatenate((pollutants_A_24hr_avg, pollutants_B_8hr_avg), axis=1)                             # Original code to be executed on the CPU
pollutants = dpnp.asnumpy(dpnp.concatenate((dpnp.asarray(pollutants_A_24hr_avg), dpnp.asarray(pollutants_B_8hr_avg)), axis=1)) # DPNP usage of concatenate
```
...
sub_indices = np.stack((vcompute_indices('PM2.5', pollutants[..., 0]),
                        ...
                        vcompute_indices('O3', pollutants[..., 6])), axis=1)               # Original code to be executed on the CPU
sub_indices = dpnp.asnumpy(dpnp.stack((dpnp.asarray(vcompute_indices('PM2.5', pollutants[..., 0])),
                        dpnp.asarray(vcompute_indices('PM10', pollutants[..., 1])),
                        dpnp.asarray(vcompute_indices('NO2', pollutants[..., 2])),
                        dpnp.asarray(vcompute_indices('NH3', pollutants[..., 3])),
                        dpnp.asarray(vcompute_indices('SO2', pollutants[..., 4])),
                        dpnp.asarray(vcompute_indices('CO', pollutants[..., 5])),
                        dpnp.asarray(vcompute_indices('O3', pollutants[..., 6]))), axis=1)) # DPNP usage of stack
```
...
```{code-cell} ipython3
# aqi_array = np.max(sub_indices, axis=1)                             # Original code to be executed on the CPU
aqi_array = dpnp.asnumpy(dpnp.max(dpnp.asarray(sub_indices), axis=1)) # DPNP usage of max
```
...
after_lock = dpnp.asnumpy(aqi_array[dpnp.where(dpnp.asarray(datetime) >= np.datetime64('2020-03-24T00'))])

# before_lock = aqi_array[np.where(datetime <= np.datetime64('2020-03-21T00'))][-(after_lock.shape[0]):]
before_lock = dpnp.asnumpy(aqi_array[dpnp.where(dpnp.asarray(datetime) <= np.datetime64('2020-03-21T00'))][-(after_lock.shape[0]):])
...
def t_test(x, y):
    diff = y - x
    var = dpnp.asnumpy(dpnp.var(dpnp.asarray(diff), ddof=1))
    num = dpnp.asnumpy(dpnp.mean(dpnp.asarray(diff)))
    denom = dpnp.asnumpy(dpnp.sqrt(dpnp.asarray(var) / len(x)))
    return dpnp.asnumpy(dpnp.divide(dpnp.asarray(num), dpnp.asarray(denom)))
