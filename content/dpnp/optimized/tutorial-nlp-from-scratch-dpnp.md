...
For the Data Parallel Extensions for NumPy, you have to install the DPNP package.

```python
! pip install "dpnp >= 0.18.0"
```

Then we check whether DPNP actually works on the system, and where (which device) get the allocations are located by default.
...
test_variable = None
print ("Testing whether we can allocate variables on GPU")
try:
    test_variable = dpnp.asarray ([1,2,3], device="gpu")
except Exception:
    print ("Error! GPU device is not available.")
    sys.exit (0)

if ":gpu:" in str(test_variable.device):
    print ("Successfully allocated `test_variable` on GPU")
else:
    print ("Error! Failed to allocate `test_variable` on GPU")
    sys.exit(0)

test_variable_def_location = dpnp.asarray([1,2,3])
if ":gpu:" in str(test_variable_def_location.device):
    print ("DPNP variables are allocated on the GPU by default.")
elif ":cpu:" in str(test_variable_def_location.device):
    print ("DPNP variables are allocated on the CPU by default.")
else:
    print ("Error! Cannot determine the default location for DPNP variables.")
    sys.exit(0)
```
...
class TextPreprocess:
    ...
    def loadGloveModel(self, emb_path):
        ...
        for line in f:
            ...
            wordEmbedding = dpnp.array([float(value) for value in splitLines[1:]])
    ...
    def text_to_paras(self, text, para_len):
        ...
        no_paras = int(dpnp.ceil(dpnp.asarray(len(words))/para_len)) ## HSG
        ...
        # paras = dpnp.array([' '.join(sents) for sents in agg_sentences]) ## HSG
        paras = np.array([' '.join(sents) for sents in agg_sentences]) ## HSG
...
def initialise_params(hidden_dim, input_dim):
    # forget gate
    Wf = dpnp.asarray(rng.standard_normal(size=(hidden_dim, hidden_dim + input_dim)))
    bf = dpnp.asarray(rng.standard_normal(size=(hidden_dim, 1)))
    # input gate
    Wi = dpnp.asarray(rng.standard_normal(size=(hidden_dim, hidden_dim + input_dim)))
    bi = dpnp.asarray(rng.standard_normal(size=(hidden_dim, 1)))
    # candidate memory gate
    Wcm = dpnp.asarray(rng.standard_normal(size=(hidden_dim, hidden_dim + input_dim)))
    bcm = dpnp.asarray(rng.standard_normal(size=(hidden_dim, 1)))
    # output gate
    Wo = dpnp.asarray(rng.standard_normal(size=(hidden_dim, hidden_dim + input_dim)))
    bo = dpnp.asarray(rng.standard_normal(size=(hidden_dim, 1)))

    # fully connected layer for classification
    W2 = dpnp.asarray(rng.standard_normal(size=(1, hidden_dim)))
    # b2 = np.zeros((1, 1))
    b2 = dpnp.zeros((1, 1))
...
def sigmoid(x):
    ...
    n = dpnp.exp(dpnp.fmin(x, 0))
    # d = (1 + np.exp(-np.abs(x)))
    d = (1 + dpnp.exp(-dpnp.abs(x)))
    return n / d
```
...
def fp_forget_gate(concat, parameters):
    ...
    #              + parameters['bf'])
    ft = sigmoid(dpnp.dot(parameters['Wf'], concat)
                 + parameters['bf'])
    return ft
```
...
def fp_input_gate(concat, parameters):
    ...
    #             + parameters['bi'])
    it = sigmoid(dpnp.dot(parameters['Wi'], concat)
                 + parameters['bi'])
    # cmt = np.tanh(np.dot(parameters['Wcm'], concat)
    #              + parameters['bcm'])
    cmt = dpnp.tanh(dpnp.dot(parameters['Wcm'], concat)
                  + parameters['bcm'])
    return it, cmt
```
...
def fp_output_gate(concat, next_cs, parameters):
    ...
    #             + parameters['bo'])
    ot = sigmoid(dpnp.dot(parameters['Wo'], concat)
                 + parameters['bo'])
    # next_hs = ot * np.tanh(next_cs)
    next_hs = ot * dpnp.tanh(next_cs)
    return ot, next_hs
```
...
def fp_fc_layer(last_hs, parameters):
    ...
    #      + parameters['b2'])
    z2 = (dpnp.dot(parameters['W2'], last_hs)
          + parameters['b2'])
    a2 = sigmoid(z2)
    return a2
```
...
def forward_prop(X_vec, parameters, input_dim):
    ...
    prev_hs = dpnp.zeros((hidden_dim, 1))
    # prev_cs = np.zeros(prev_hs.shape)
    prev_cs = dpnp.zeros(prev_hs.shape)
...
def initialize_grads(parameters):
    ...
    for param in parameters.keys():
        ...
        grads[f'd{param}'] = dpnp.zeros((parameters[param].shape))
    return grads
```
...
def bp_forget_gate(hidden_dim, concat, dh_prev, dc_prev, cache, gradients, parameters):
    ...
    dft = ((dc_prev * cache["prev_cs"] + cache["ot"]
           * (1 - dpnp.square(dpnp.tanh(cache["next_cs"])))
           * cache["prev_cs"] * dh_prev) * cache["ft"] * (1 - cache["ft"]))
    # dWf = dft * dft/dWf
    # gradients['dWf'] += np.dot(dft, concat.T)
    gradients['dWf'] += dpnp.dot(dft, concat.T)
    # dbf = dft * dft/dbf
    # gradients['dbf'] += np.sum(dft, axis=1, keepdims=True)
    gradients['dbf'] += dpnp.sum(dft, axis=1, keepdims=True)
    # dh_f = dft * dft/dh_prev
    # dh_f = np.dot(parameters["Wf"][:, :hidden_dim].T, dft)
    dh_f = dpnp.dot(parameters["Wf"][:, :hidden_dim].T, dft)
...
def bp_input_gate(hidden_dim, concat, dh_prev, dc_prev, cache, gradients, parameters):
    ...
    dcmt = ((dc_prev * cache["it"] + cache["ot"]
            * (1 - dpnp.square(dpnp.tanh(cache["next_cs"])))
            * cache["it"] * dh_prev) * (1 - dpnp.square(cache["cmt"])))
    # dWi = dit * dit/dWi
    # gradients['dWi'] += np.dot(dit, concat.T)
    gradients['dWi'] += dpnp.dot(dit, concat.T)
    # dWcm = dcmt * dcmt/dWcm
    # gradients['dWcm'] += np.dot(dcmt, concat.T)
    gradients['dWcm'] += dpnp.dot(dcmt, concat.T)
    # dbi = dit * dit/dbi
    # gradients['dbi'] += np.sum(dit, axis=1, keepdims=True)
    gradients['dbi'] += dpnp.sum(dit, axis=1, keepdims=True)
    # dWcm = dcmt * dcmt/dbcm
    # gradients['dbcm'] += np.sum(dcmt, axis=1, keepdims=True)
    gradients['dbcm'] += dpnp.sum(dcmt, axis=1, keepdims=True)
    # dhi = dit * dit/dh_prev
    # dh_i = np.dot(parameters["Wi"][:, :hidden_dim].T, dit)
    dh_i = dpnp.dot(parameters["Wi"][:, :hidden_dim].T, dit)
    # dhcm = dcmt * dcmt/dh_prev
    # dh_cm = np.dot(parameters["Wcm"][:, :hidden_dim].T, dcmt)
    dh_cm = dpnp.dot(parameters["Wcm"][:, :hidden_dim].T, dcmt)
...
def bp_output_gate(hidden_dim, concat, dh_prev, dc_prev, cache, gradients, parameters):
    ...
    #       * cache["ot"] * (1 - cache["ot"]))
    dot = (dh_prev * dpnp.tanh(cache["next_cs"])
           * cache["ot"] * (1 - cache["ot"]))
    # dWo = dot * dot/dWo
    # gradients['dWo'] += np.dot(dot, concat.T)
    gradients['dWo'] += dpnp.dot(dot, concat.T)
    # dbo = dot * dot/dbo
    # gradients['dbo'] += np.sum(dot, axis=1, keepdims=True)
    gradients['dbo'] += dpnp.sum(dot, axis=1, keepdims=True)
    # dho = dot * dot/dho
    # dh_o = np.dot(parameters["Wo"][:, :hidden_dim].T, dot)
    dh_o = dpnp.dot(parameters["Wo"][:, :hidden_dim].T, dot)
...
def backprop(y, caches, hidden_dim, input_dim, time_steps, parameters):
    ...
    dc_prev = dpnp.zeros((dh_prev.shape))
    ...
    for t in reversed(range(time_steps)):
        ...
        concat = dpnp.concatenate((cache["prev_hs"], cache["xt"]), axis=0)
...
def initialise_mav(hidden_dim, input_dim, params):
    ...
    for key in params:
        ...
        v['d' + key] = dpnp.zeros(params[key].shape)
        # s['d' + key] = np.zeros(params[key].shape)
        s['d' + key] = dpnp.zeros(params[key].shape)
...
def update_parameters(parameters, gradients, v, s,
    ...
    for key in parameters:
        ...
        parameters[key] = (parameters[key] - learning_rate
                           * v['d' + key] / dpnp.sqrt(s['d' + key] + 1e-8))
...
def loss_f(A, Y):
    ...
    loss = (- Y * dpnp.log(A + epsilon)
            - (1 - Y) * dpnp.log(1 - A + epsilon))
    # Return loss
    return dpnp.squeeze(loss)
```
...
for epoch in range(epochs):
    ...
    mean_train_cost = float(dpnp.mean(dpnp.asarray(train_j))) # HSG
...
for param_key in parameters:
    np_parameters[param_key] = dpnp.asnumpy(parameters[param_key])
np.save('tutorial-nlp-from-scratch/parameters.npy', np_parameters)
```
...
if os.path.isfile('tutorial-nlp-from-scratch/parameters.npy'):
    ...
    parameters_np = np.load('tutorial-nlp-from-scratch/parameters.npy', allow_pickle=True).item()
    parameters = dict()
    for key in parameters_np:
        parameters[key] = dpnp.asarray(parameters_np[key])
...
for index, text in enumerate(X_pred):
    ...
    threshold = 0.5
    # preds = dpnp.array(preds)
    preds = dpnp.asarray(preds)
    # Mark all predictions > threshold as positive and < threshold as negative
    # pos_indices = np.where(preds > threshold)  # indices where output > 0.5 # HSG
    pos_indices = np.where(dpnp.asnumpy(preds) > threshold)  # indices where output > 0.5 # HSG
    # neg_indices = np.where(preds < threshold)  # indices where output < 0.5 # HSG
    neg_indices = np.where(dpnp.asnumpy(preds) < threshold)  # indices where output < 0.5 # HSG

    # HSG using dpnp.where() leads to issues later in following statement
