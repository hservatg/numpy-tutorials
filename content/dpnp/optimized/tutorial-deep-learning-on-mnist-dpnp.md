...
# )
x_train, y_train, x_test, y_test = (
    dpnp.asarray(mnist_dataset["training_images"]),
    dpnp.asarray(mnist_dataset["training_labels"]),
    dpnp.asarray(mnist_dataset["test_images"]),
    dpnp.asarray(mnist_dataset["test_labels"]),
)
```
...
plt.imshow(dpnp.asnumpy(mnist_image), cmap="gray")
# Display the image.
plt.show()
```
...
for sample, ax in zip(rng.choice(dpnp.asnumpy(x_train), size=num_examples, replace=False), axes):
    ax.imshow(dpnp.asnumpy(sample.reshape(28, 28)), cmap="gray")
```
...
def one_hot_encoding(labels, dimension=10):
    ...
    one_hot_labels = labels[..., None] == dpnp.arange(dimension)[None]
...
weights_1 = dpnp.asarray(0.2 * rng.random((pixels_per_image, hidden_size)) - 0.1)
# weights_2 = 0.2 * rng.random((hidden_size, num_labels)) - 0.1
weights_2 = dpnp.asarray(0.2 * rng.random((hidden_size, num_labels)) - 0.1)
```
...
for j in range(epochs):
    ...
    for i in range(len(training_images)):
        ...
        layer_1 = dpnp.dot(layer_0, weights_1)
        # 3. Pass the hidden layer's output through the ReLU activation function.
        layer_1 = relu(layer_1)
        # 4. Define the dropout function for regularization.
        dropout_mask = dpnp.asarray(rng.integers(low=0, high=2, size=layer_1.shape))
        ...
        layer_2 = dpnp.dot(layer_1, weights_2)

        # Backpropagation/backward pass:
        # 1. Measure the training error (loss function) between the actual
        #    image labels (the truth) and the prediction by the model.
        # training_loss += np.sum((training_labels[i] - layer_2) ** 2)
        training_loss += dpnp.sum((training_labels[i] - layer_2) ** 2)
        # 2. Increment the accurate prediction count.
        training_accurate_predictions += int(
        #    np.argmax(layer_2) == np.argmax(training_labels[i])
             dpnp.argmax(layer_2) == dpnp.argmax(training_labels[i])
        )
        # 3. Differentiate the loss function/error.
        layer_2_delta = training_labels[i] - layer_2
        # 4. Propagate the gradients of the loss function back through the hidden layer.
        # layer_1_delta = np.dot(weights_2, layer_2_delta) * relu2deriv(layer_1)
        layer_1_delta = dpnp.dot(weights_2, layer_2_delta) * relu2deriv(layer_1)
        # 5. Apply the dropout to the gradients.
        layer_1_delta *= dropout_mask
        # 6. Update the weights for the middle and input layers
        #    by multiplying them by the learning rate and the gradients.
        # weights_1 += learning_rate * np.outer(layer_0, layer_1_delta)
        weights_1 += learning_rate * dpnp.outer(layer_0, layer_1_delta)
        # weights_2 += learning_rate * np.outer(layer_1, layer_2_delta)
        weights_2 += learning_rate * dpnp.outer(layer_1, layer_2_delta)
    ...
    test_loss = dpnp.sum((test_labels - results) ** 2)

    # Measure prediction accuracy on test set
    # test_accurate_predictions = np.sum(
    test_accurate_predictions = dpnp.sum(
        # np.argmax(results, axis=1) == np.argmax(test_labels, axis=1)
        dpnp.argmax(results, axis=1) == dpnp.argmax(test_labels, axis=1)
    )
    ...
    print(
        (
            f"Epoch: {j}\n"
            f"  Training set error: {dpnp.asnumpy(training_loss / len(training_images)):.3f}\n"
            f"  Training set accuracy: {training_accurate_predictions / len(training_images)}\n"
            f"  Test set error: {dpnp.asnumpy(test_loss / len(test_images)):.3f}\n"
...
training_metrics = {
    ...
    "accuracy": dpnp.asarray(store_training_accurate_pred) / len(training_images),
    "error": dpnp.asarray(store_training_loss) / len(training_images),
}
...
test_metrics = {
    ...
    "accuracy": dpnp.asarray(store_test_accurate_pred) / len(test_images),
    "error": dpnp.asarray(store_test_loss) / len(test_images),
}
...
for ax, metrics, title in zip(
    ...
    for metric, values in metrics.items():
        ax.plot(epoch_range, dpnp.asnumpy(values), label=metric.capitalize())
    ax.set_title(title)
