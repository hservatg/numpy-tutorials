...
img_gray = img_array @ dpnp.asarray([0.2126, 0.7152, 0.0722])
```
...
plt.imshow(dpnp.asnumpy(img_gray), cmap="gray")
plt.show()
```
...
U, s, Vt = dpnp.linalg.svd(img_gray)
```
...
Sigma = dpnp.zeros((U.shape[1], Vt.shape[0]))
dpnp.fill_diagonal(Sigma, s)
```
...
dpnp.linalg.norm(img_gray - U @ Sigma @ Vt)
```
...
plt.imshow(dpnp.asnumpy(approx), cmap="gray")
plt.show()
```
...
img_array_transposed = dpnp.transpose(img_array, (2, 0, 1))
...
U, s, Vt = dpnp.linalg.svd(img_array_transposed)
```
...
Sigma = dpnp.zeros((3, 768, 1024))
for j in range(3):
    # np.fill_diagonal(Sigma[j, :, :], s[j, :])
    dpnp.fill_diagonal(Sigma[j, :, :], s[j, :])
```
...
reconstructed = dpnp.clip(reconstructed, 0, 1)
# plt.imshow(transpose(reconstructed, (1, 2, 0)))
plt.imshow(dpnp.asnumpy(dpnp.transpose(reconstructed, (1, 2, 0))))
plt.show()
```
...
plt.imshow(dpnp.asnumpy(dpnp.transpose(approx_img, (1, 2, 0))))
plt.show()
```
