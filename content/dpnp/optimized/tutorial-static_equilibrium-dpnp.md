...
forceA = dpnp.array([1, 0, 0])
# forceB = np.array([0, 1, 0])
forceB = dpnp.array([0, 1, 0])
...
u, v, w = dpnp.asnumpy(forceA)  # breaking the force vector into individual components
d3.quiver(x, y, z, u, v, w, color="r", label="forceA")

# u, v, w = forceB
u, v, w = dpnp.asnumpy(forceB)
...
# u, v, w = forceA
u, v, w = dpnp.asnumpy(forceA)
d3.quiver(x, y, z, u, v, w, color="r", label="forceA")
# u, v, w = forceB
u, v, w = dpnp.asnumpy(forceB)
d3.quiver(x, y, z, u, v, w, color="b", label="forceB")
# u, v, w = forceC
u, v, w = dpnp.asnumpy(forceC)
...
u, v, w = dpnp.asnumpy(forceA + forceB + R)  # add them all together for sum of forces
...
poleBase = dpnp.array([0, 0, 0])
# cordBase = np.array([3, 0, 0])
cordBase = dpnp.array([3, 0, 0])
# cordConnection = np.array([0, 0, 2])
cordConnection = dpnp.array([0, 0, 2])
...
cordUnit = cordDirection / dpnp.linalg.norm(cordDirection)
...
momentCord = dpnp.cross(forceCord, poleDirection)
...
B = dpnp.array([0, 3, 0])
C = dpnp.array([0, 6, 0])
D = dpnp.array([1.5, 0, -3])
E = dpnp.array([1.5, 0, 3])
F = dpnp.array([-3, 0, 2])
```
...
UnitBD = BD / dpnp.linalg.norm(BD)
UnitBE = BE / dpnp.linalg.norm(BE)
UnitCF = CF / dpnp.linalg.norm(CF)

RadBD = dpnp.cross(AB, UnitBD)
RadBE = dpnp.cross(AB, UnitBE)
RadCF = dpnp.cross(AC, UnitCF)
```
