...
x, y = dpnp.meshgrid(dpnp.linspace(-10, 10, 20), dpnp.linspace(-10, 10, 20))
...
output = dpnp.abs(f(mesh))  # Take the absolute value of the output (for plotting)

fig = plt.figure()
ax = plt.axes(projection='3d')

ax.scatter(dpnp.asnumpy(x), dpnp.asnumpy(y), dpnp.asnumpy(output), alpha=0.2)
...
selected_values = dpnp.asarray([0.4 + 0.4j, 0.41 + 0.4j, 0.4 + 0.41j])
num_iter = 9

# outputs = np.zeros((num_iter+1, selected_values.shape[0]), dtype=complex)
outputs = dpnp.zeros((num_iter+1, selected_values.shape[0]), dtype=complex)
...
for ax, data in zip(axes, outputs.T):
    ...
    cycle = ax.scatter(dpnp.asnumpy(data.real), dpnp.asnumpy(data.imag), c=range(data.shape[0]), alpha=0.6)
...
def divergence_rate(mesh, num_iter=10, radius=2):
    ...
    diverge_len = dpnp.zeros(mesh.shape)  # Keep tally of the number of iterations

    # Iterate on element if and only if |element| < radius (Otherwise assume divergence)
    for i in range(num_iter):
        # conv_mask = np.abs(z) < radius
        conv_mask = dpnp.abs(z) < radius
...
x, y = dpnp.meshgrid(dpnp.linspace(-2, 2, 400), dpnp.linspace(-2, 2, 400))
mesh = x + (1j * y)
...
im = ax.imshow(dpnp.asnumpy(output), extent=[-2, 2, -2, 2])
...
def julia(mesh, c=-1, num_iter=10, radius=2):
    ...
    diverge_len = dpnp.zeros(z.shape)

    for i in range(num_iter):
        # conv_mask = np.abs(z) < radius
        conv_mask = dpnp.abs(z) < radius
        # z[conv_mask] = np.square(z[conv_mask]) + c
        z[conv_mask] = dpnp.square(z[conv_mask]) + c
        diverge_len[conv_mask] += 1
...
x, y = dpnp.meshgrid(dpnp.linspace(-1, 1, 400), dpnp.linspace(-1, 1, 400))
small_mesh = x + (1j * y)
...
def plot_fractal(fractal, title='Fractal', figsize=(6, 6), cmap='rainbow', extent=[-2, 2, -2, 2]):
    ...
    im = ax.imshow(dpnp.asnumpy(fractal), extent=extent, cmap=cmap)
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.1)
    plt.colorbar(im, cax=cax, label='Number of iterations')
```
...
output = julia(mesh, c=dpnp.pi/10, num_iter=20)
...
def mandelbrot(mesh, num_iter=10, radius=2):
    ...
    z = dpnp.zeros(mesh.shape, dtype=dpnp.complex128)
    # diverge_len = np.zeros(z.shape)
    diverge_len = dpnp.zeros(z.shape)
    ...
    for i in range(num_iter):
        ...
        z[conv_mask] = dpnp.square(z[conv_mask]) + c[conv_mask]
        diverge_len[conv_mask] += 1
...
def general_julia(mesh, c=-1, f=dpnp.square, num_iter=100, radius=2):
...
for deg, ax in enumerate(axes.ravel()):
    ...
    power = lambda z: dpnp.power(z, degree)  # Create power function for current degree

    diverge_len = general_julia(mesh, f=power, num_iter=15)
    ax.imshow(dpnp.asnumpy(diverge_len), extent=[-2, 2, -2, 2], cmap='binary')
...
def f_tan(z):
    ...
    return dpnp.square(dpnp.tan(z))


def d_tan(z):
    # return 2*np.tan(z) / np.square(np.cos(z))
    return 2*dpnp.tan(z) / dpnp.square(dpnp.cos(z))
```
...
def sin_sum(z, n=10):
    ...
    total = dpnp.zeros(z.size, dtype=z.dtype)
    for i in range(1, n+1):
        # total += np.power(np.sin(z), i)
        total += dpnp.power(np.sin(z), i)
    return total
...
def d_sin_sum(z, n=10):
    ...
    for i in range(1, n+1):
        ...
        total += i * dpnp.power(dpnp.sin(z), i-1) * dpnp.cos(z)
    return total
```
...
def accident(z):
    ...
    return z - (2 * dpnp.power(dpnp.tan(z), 2) / (dpnp.sin(z) * dpnp.cos(z)))
```
