...
combined_xray_images_1 = dpnp.array(
...
for img, ax in zip(combined_xray_images_1, axes):
    ax.imshow(dpnp.asnumpy(img), cmap='gray')
    ax.axis('off')
```
...
imageio.mimwrite(GIF_PATH, dpnp.asnumpy(combined_xray_images_1), format= ".gif", duration=1000)
```
...
xray_image_laplace_gaussian = ndimage.gaussian_laplace(dpnp.asnumpy(xray_image), sigma=1)
```
...
```{code-cell}
x_ray_image_gaussian_gradient = ndimage.gaussian_gradient_magnitude(dpnp.asnumpy(xray_image), sigma=2)
```
...
xray_image_sobel = dpnp.hypot(dpnp.asarray(x_sobel), dpnp.asarray(y_sobel))
...
axes[1].imshow(dpnp.asnumpy(xray_image_sobel), cmap="gray")
axes[2].set_title("Sobel (edges) - CMRmap")
axes[2].imshow(dpnp.asnumpy(xray_image_sobel), cmap="CMRmap")
for i in axes:
    i.axis("off")
plt.show()
```
...
xray_image_canny = dpnp.hypot(dpnp.asarray(x_prewitt), dpnp.asarray(y_prewitt))

xray_image_canny *= 255.0 / dpnp.max(xray_image_canny)
...
axes[1].imshow(dpnp.asnumpy(xray_image_canny), cmap="prism")
axes[2].set_title("Canny (edges) - nipy_spectral")
axes[2].imshow(dpnp.asnumpy(xray_image_canny), cmap="nipy_spectral")
axes[3].set_title("Canny (edges) - terrain")
axes[3].imshow(dpnp.asnumpy(xray_image_canny), cmap="terrain")
for i in axes:
    i.axis("off")
plt.show()
```
...
xray_image_mask_noisy = dpnp.where(xray_image > 150, xray_image, 0)
...
axes[1].imshow(dpnp.asnumpy(xray_image_laplace_gaussian), cmap="gray")
axes[2].set_title("Gaussian gradient (edges)")
axes[2].imshow(dpnp.asnumpy(x_ray_image_gaussian_gradient), cmap="gray")
axes[3].set_title("Sobel (edges) - grayscale")
axes[3].imshow(dpnp.asnumpy(xray_image_sobel), cmap="gray")
axes[4].set_title("Sobel (edges) - hot")
axes[4].imshow(dpnp.asnumpy(xray_image_sobel), cmap="hot")
axes[5].set_title("Canny (edges) - prism)")
axes[5].imshow(dpnp.asnumpy(xray_image_canny), cmap="prism")
axes[6].set_title("Canny (edges) - nipy_spectral)")
axes[6].imshow(dpnp.asnumpy(xray_image_canny), cmap="nipy_spectral")
axes[7].set_title("Mask (> 150, noisy)")
axes[7].imshow(dpnp.asnumpy(xray_image_mask_noisy), cmap="gray")
axes[8].set_title("Mask (> 150, less noisy)")
axes[8].imshow(dpnp.asnumpy(xray_image_mask_less_noisy), cmap="gray")
for i in axes:
    i.axis("off")
plt.show()
```
