...
def initialise_params(hidden_dim, input_dim):
    ...
    # b2 = np.zeros((1, 1))
    b2 = dpnp.asnumpy(dpnp.zeros((1, 1)))
...
def fp_forget_gate(concat, parameters):
    ...
    #              + parameters['bf'])
    ft = sigmoid(dpnp.asnumpy(dpnp.dot(dpnp.asarray(parameters['Wf']), dpnp.asarray(concat))
                 + dpnp.asarray(parameters['bf'])))
    return ft
```
...
def fp_input_gate(concat, parameters):
    ...
    #              + parameters['bi'])
    it = sigmoid(dpnp.asnumpy(dpnp.dot(dpnp.asarray(parameters['Wi']), dpnp.asarray(concat))
                 + dpnp.asarray(parameters['bi'])))
    # cmt = np.tanh(np.dot(parameters['Wcm'], concat)
    #              + parameters['bcm'])
    cmt = dpnp.asnumpy(dpnp.tanh(dpnp.dot(dpnp.asarray(parameters['Wcm']), dpnp.asarray(concat))
                  + dpnp.asarray(parameters['bcm'])))
    return it, cmt
```
...
def fp_fc_layer(last_hs, parameters):
    ...
    #      + parameters['b2'])
    z2 = dpnp.asnumpy(dpnp.dot(dpnp.asarray(parameters['W2']), dpnp.asarray(last_hs))
          + dpnp.asarray(parameters['b2']))
    a2 = sigmoid(z2)
    return a2
```
