...
```{code-cell} ipython3
def f(z):
#    return np.square(z) - 1                # Original version
    return dpnp.asnumpy(dpnp.square(dpnp.asarray(z)) - 1) # DPNP version using square
```
...
x, y = (dpnp.asnumpy(v) for v in dpnp.meshgrid(dpnp.linspace(-10, 10, 20), dpnp.linspace(-10, 10, 20)))
...
output = dpnp.asnumpy(dpnp.abs(dpnp.asarray(f(mesh))))  # Take the absolute value of the output (for plotting)
...
output = dpnp.asnumpy(dpnp.abs(dpnp.asarray(f(f(mesh)))))
...
outputs = dpnp.asnumpy(dpnp.zeros((num_iter+1, selected_values.shape[0]), dtype=complex))
...
def divergence_rate(mesh, num_iter=10, radius=2):
    ...
    diverge_len = dpnp.asnumpy(dpnp.zeros(mesh.shape))  # Keep tally of the number of iterations
...
x, y = (dpnp.asnumpy(v) for v in dpnp.meshgrid(dpnp.linspace(-2, 2, 400), dpnp.linspace(-2, 2, 400)))
mesh = x + (1j * y)
...
x, y = (dpnp.asnumpy(v) for v in dpnp.meshgrid(dpnp.linspace(-1, 1, 400), dpnp.linspace(-1, 1, 400)))
small_mesh = x + (1j * y)
...
def mandelbrot(mesh, num_iter=10, radius=2):
    ...
    z = dpnp.asnumpy(dpnp.zeros(mesh.shape, dtype=np.complex128))
    diverge_len = dpnp.asnumpy(dpnp.zeros(z.shape))
    ...
    for i in range(num_iter):
        ...
        z[conv_mask] = dpnp.asnumpy(dpnp.square(dpnp.asarray(z[conv_mask])) + dpnp.asarray(c[conv_mask]))
        diverge_len[conv_mask] += 1
...
for deg, ax in enumerate(axes.ravel()):
    ...
    power = lambda z: dpnp.asnumpy(dpnp.power(dpnp.asarray(z), degree))  # Create power function for current degree
...
def newton_fractal(mesh, f, df, num_iter=10, r=2):
    ...
    for i in range(num_iter):
        ...
        conv_mask = dpnp.asnumpy(np.abs(dpnp.asarray(z)) < r)
        pz = f(z[conv_mask])
        dp = df(z[conv_mask])
...
p = dpnp.polynomial.Polynomial([-16, 0, 0, 0, 15, 0, 0, 0, 1])
p
```
...
def f_tan(z):
    ...
    return dpnp.asnumpy(dpnp.square(dpnp.tan(dpnp.asarray(z))))


def d_tan(z):
    # return 2*np.tan(z) / np.square(np.cos(z))
    return 2*(dpnp.asnumpy(dpnp.tan(dpnp.asarray(z)))) / dpnp.asnumpy(dpnp.square(dpnp.cos(dpnp.asarray(z))))
```
...
def sin_sum(z, n=10):
    ...
    total = dpnp.asnumpy(dpnp.zeros(z.size, dtype=z.dtype))
    for i in range(1, n+1):
        # total += np.power(np.sin(z), i)
        total += dpnp.asnumpy(dpnp.power(dpnp.sin(dpnp.asarray(z)), i))
    return total
...
def d_sin_sum(z, n=10):
    ...
    for i in range(1, n+1):
        ...
        total += i * dpnp.asnumpy(dpnp.power(dpnp.sin(dpnp.asarray(z)), i-1) * dpnp.cos(dpnp.asarray(z)))
    return total
```
...
def f(z):
    ...
    return dpnp.asnumpy(dpnp.tan(dpnp.square(dpnp.asarray(z))))
```
...
def g(z):
    # return np.sin(f(z))
    return dpnp.asnumpy(dpnp.sin(dpnp.asarray(f(z))))
```
...
def accident(z):
    ...
    return z - dpnp.asnumpy(2 * dpnp.power(dpnp.tan(dpnp.asarray(z)), 2) / (dpnp.sin(dpnp.asarray(z)) * dpnp.cos(dpnp.asarray(z))))
```
