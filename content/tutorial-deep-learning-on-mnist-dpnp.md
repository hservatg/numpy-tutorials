...
for j in range(epochs):
    ...
    for i in range(len(training_images)):
        ...
        # layer_1 = np.dot(layer_0, weights_1)                                           # Original code to be executed on the CPU
        layer_1 = dpnp.asnumpy(dpnp.dot(dpnp.asarray(layer_0), dpnp.asarray(weights_1))) # Use DPNP variants of dot
        ...
        # layer_2 = np.dot(layer_1, weights_2)                                           # Original code to be executed on the CPU
        layer_2 = dpnp.asnumpy(dpnp.dot(dpnp.asarray(layer_1), dpnp.asarray(weights_2))) # Use DPNP variants of dot

        # Backpropagation/backward pass:
        # 1. Measure the training error (loss function) between the actual
        #    image labels (the truth) and the prediction by the model.
        # training_loss += np.sum((training_labels[i] - layer_2) ** 2)                             # Original code to be executed on the CPU
        training_loss += dpnp.asnumpy(dpnp.sum(dpnp.asarray((training_labels[i] - layer_2) ** 2))) # Use DPNP variant of sum -- NOTE! **2 is on the CPU!?
        # 2. Increment the accurate prediction count.
        training_accurate_predictions += int(
            # np.argmax(layer_2) == np.argmax(training_labels[i])                               # Original code to be executed on the CPU
            dpnp.argmax(dpnp.asarray(layer_2)) == dpnp.argmax(dpnp.asarray(training_labels[i])) # Use DPNP variant for argmax
        )
        # 3. Differentiate the loss function/error.
        layer_2_delta = training_labels[i] - layer_2
        # 4. Propagate the gradients of the loss function back through the hidden layer.
        # layer_1_delta = np.dot(weights_2, layer_2_delta) * relu2deriv(layer_1)                                           # Original code to be executed on the CPU
        layer_1_delta = dpnp.asnumpy(dpnp.dot(dpnp.asarray(weights_2), dpnp.asarray(layer_2_delta))) * relu2deriv(layer_1) # Use DPNP variant of dot, but multiplication is on the host!?
        # 5. Apply the dropout to the gradients.
        layer_1_delta *= dropout_mask
        # 6. Update the weights for the middle and input layers
        #    by multiplying them by the learning rate and the gradients.
        # weights_1 += learning_rate * np.outer(layer_0, layer_1_delta)                                            # Original code to be executed on the CPU
        weights_1 += learning_rate * dpnp.asnumpy(dpnp.outer(dpnp.asarray(layer_0), dpnp.asarray(layer_1_delta)))  # Use DPNP variant of outer
        # weights_2 += learning_rate * np.outer(layer_1, layer_2_delta)                                            # Original code to be executed on the CPU
        weights_2 += learning_rate * dpnp.asnumpy(dpnp.outer(dpnp.asarray(layer_1), dpnp.asarray(layer_2_delta)))  # Use DPNP variant of outer
