...
```{code-cell}
# img_array.max(), img_array.min()                           # Original code to be executed on the CPU
dpnp.asarray(img_array).max(), dpnp.asarray(img_array).min() # Use DPNP variants of max/min
```
...
```{code-cell}
# linalg.norm(img_gray - U @ Sigma @ Vt)                                                                             # Original code to be executed on the CPU
dpnp.linalg.norm(dpnp.array(img_gray) - dpnp.matmul( dpnp.matmul(dpnp.array(U),dpnp.array(Sigma)), dpnp.array(Vt)))  # Use DPNP variant of matmul (@ operator)
```
...
```{code-cell}
# np.allclose(img_gray, U @ Sigma @ Vt)                                                                                # Original code to be executed on the CPU
np.allclose(img_gray, dpnp.asnumpy ( dpnp.matmul ( dpnp.matmul( dpnp.array(U), dpnp.array(Sigma)), dpnp.array(Vt)) ) ) # Use DPNP variant of matmul (@ operator)
```
...
```{code-cell}
# approx = U @ Sigma[:, :k] @ Vt[:k, :]                                                                                  # Original code to be executed on the CPU
approx = dpnp.asnumpy ( dpnp.matmul ( dpnp.matmul (dpnp.array (U), dpnp.array (Sigma[:,:k]) ), dpnp.array(Vt[:k, :]) ) ) # Use DPNP variant of matmul
```
...
```
# np.transpose(x, axes=(i, j, k))             # Original code to be executed on the CPU
dpnp.transpose(dpnp.array(x), axes=(i, j, k)) # Use DPNP variant of transpose
```
...
img_array_transposed = dpnp.asnumpy (dpnp.transpose(dpnp.array(img_array), (2, 0, 1)) )
...
```{code-cell}
# Sigma = np.zeros((3, 768, 1024))                  # Original code to be executed on the CPU
...
```{code-cell}
# reconstructed = U @ Sigma @ Vt                                                                                  # Original code to be executed on the CPU
reconstructed = dpnp.asnumpy ( dpnp.matmul ( dpnp.matmul (dpnp.array (U), dpnp.array (Sigma)), dpnp.array(Vt) ) ) # Use DPNP variant of matmul
```
...
```{code-cell}
reconstructed.min(), reconstructed.max()                             # Original code to be executed on the CPU
dpnp.asarray(reconstructed).min(), dpnp.asarray(reconstructed).max() # Use DPNP variants of max/min
```

Since `imshow` expects values in the range, we can use `clip` to excise the floating point error:

```{code-cell}
# reconstructed = np.clip(reconstructed, 0, 1)                           # Original code to be executed on the CPU
reconstructed = dpnp.asnumpy(dpnp.clip (dpnp.array(reconstructed), 0, 1)) # Use DPNP variant of clip

# plt.imshow(np.transpose(reconstructed, (1, 2, 0)))                                 # Original code to be executed on the CPU
plt.imshow (dpnp.asnumpy(dpnp.transpose (dpnp.array(reconstructed), (1, 2, 0) )))    # Use DPNP variant of transpose
plt.show()
```
...
```{code-cell}
# approx_img = U @ Sigma[..., :k] @ Vt[..., :k, :]                                                                               # Original code to be executed on the CPU
approx_img = dpnp.asnumpy ( dpnp.matmul ( dpnp.matmul (dpnp.array(U), dpnp.array(Sigma[...,:k]) ), dpnp.array(Vt[..., :k, :])) ) # Use DPNP variant of matmul
```
...
```{code-cell}
# plt.imshow(np.transpose(approx_img, (1, 2, 0)))                           # Original code to be executed on the CPU
plt.imshow(dpnp.asnumpy(dpnp.transpose(dpnp.array(approx_img), (1, 2, 0)))) # Use DPNP variant of transpose
plt.show()
```
